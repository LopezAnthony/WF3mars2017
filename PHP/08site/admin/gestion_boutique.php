...
//-------------------------------TRAITEMENT----------------------------------
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'suppression' && isset($_GET['id_produit'])){
        ...
        if(!empty($produit_a_supprimer['photo']) && file_exists($chemin_photo_a_supprimer)){
            //si il y a un chemin de photo en base ET que le fichier existe (file_exists est une fonction prédéfinie), on peu le supprimer :
    ...
    if($_POST){ //équivalent à !empty($_POST) car si le $_POST est rempli, il vaut TRUE = formulaire posté
        ...
        //9.Modifiacation de la photo (suite) :
        ...
        //.4 suite de l'enregistrement en BDD : 
            ...
            $contenu .= '<div class="bg-success">Le prodtui a été enregistré</div>';
            ...
            $contenu .= '<table class="table">';
                $contenu .= '<tr>';
                    for($i = 0; $i < $resultat->columnCount(); $i++){
                        ...
                        $contenu .= '<th>' . $colonne['name'] . '</th>'; //getColumnMeta() retournne un array contenant notamment l'indice name contenant le nom de la colonne.
                    }
                    ...
                    while ($ligne = $resultat->fetch(PDO::FETCH_ASSOC)){
                        $contenu .= '<tr>';
                            foreach($ligne as $index => $data){ // $index réceptionne les indices, $data les valeurs
                                ...
                                if( $index ==  'photo'){
                                    $contenu .= '<td><img src="'.$data.'" width="70" height="70" alt="image de produit"></td>';
                                }else{
                                    $contenu .= '<td>'.$data.'</td>';
                                }
                            }
                            $contenu .= '<td><a href="?action=modification&id_produit='.$ligne['id_produit'].'">Modifier</a> / <a href="?action=suppression&id_produit='.$ligne['id_produit'].'" onclick="return(confirm(\'Etes-vous certain de vouloir supprimer ce produit ? \'));">Supprimer</a></td>';
                        $contenu .= '</tr>';
                    }
            $contenu .= '</table>';
        }
...
<h3>Formulaire d'ajoute ou de modification d'un produit</h3>
<form method="post" enctype="multipart/form-data" action=""> <!-- "multipart/form-data" permet d'uploader un fichier et de générer une superglobale $_FILES -->
    ...
    <input type="radio" name="public" value="f" <?php if(isset($produit_actuel['public']) && $produit_actuel['public'] == 'm'){ echo 'checked'; } ?>> Femme
    <input type="radio" name="public" value="mixte" <?php if(isset($produit_actuel['public']) && $produit_actuel['public'] == 'f'){ echo 'checked'; } ?>> Mixte <br><br>

    <label for="photo">Photo</label><br><br>
    <input type="file" id="photo" name="photo"><br> <!-- coupler l'attribut enctype="multipart/form-date" dela balise <form>, le type 'file' permet d'uploader un fichier (ici une photo) -->
    ...
    <input type="submit" value="valider" class="brn">
