...
--***************************************
    ...
    --Les vues (ou tables virtuelles) sont des objets de la base de donnée, constitué d'un nom et d'une requête de sélection

        --Une fois une vue définie, on peut l'utiliser comme on le ferait avec une table, laquelle serait constituée des donées sélectionnées par la requête définissant la vue.
            USE entreprise;

        --Création d'une vue :
            CREATE VIEW vue_homme AS SELECT prenom, nom, sexe, service FROM employes WHERE sexe = 'm';
            --Crée un table virtuelle (ou vue) remplie avec les données du SELECT
            ...
            --Si il y a un changement dans la table d'origine, la vue est corrigée dynamiquement car  elle enregistre la requête SELECT qui pointe vers la table d'origine. Inversemment, si il y a un changement dans la table virtuelle, il s'impact dans la table d'origine.
            --Il y a intérêt à faire des vues en termes de gain de ressources, ou quand il y a des requêtes complexe a exécuter.
