...
if($mode == 'enregistrer'){
    ...
    // => setInterval
    $liste = $pdo->query("SELECT * FROM article");
    while($resultat = $liste->fetch(PDO::FETCH_ASSOC)){
        $tab['resultat'] .= '<li>Titre : '. $resultat['titre'] .'</li>';
        $tab['resultat'] .= '<li>Contenu : '. $resultat['contenu'] .'</li>';
        $tab['resultat'] .= '<li>Auteur : '. $resultat['auteur'] .'</li>';
    }
}
